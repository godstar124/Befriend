...
- (void)viewDidLoad
{
    ...
    if (!self.isNotFirst) {
        ...
        [button setImage:[UIImage imageNamed:@"itemMusic.png"] forState:UIControlStateNormal];
        [button addTarget:self action:@selector(clickMusic) forControlEvents:UIControlEventTouchUpInside];
        UIBarButtonItem *rightItem = [[UIBarButtonItem alloc] initWithCustomView:button];
        self.navigationItem.rightBarButtonItem = rightItem;
        
        UIButton *button1 = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 53, 44)];
        [button1 setImage:[UIImage imageNamed:@"itemSportB.png"] forState:UIControlStateNormal];
    ...
    namesArray = [NSArray arrayWithObjects:@"Comedy", @"Action", @"Adventure", @"Drama", @"Romantic", @"Documentary", @"Horror", @"Science Fiction", @"Fantasy", @"Historical", @"Animated", @"Musicals", @"Family", @"War", @"Independent", @"Arthouse", @"Erotic", @"Manga", @"Short Films", @"Western", @"Foreign", @"Mystery", @"Thriller", @"Others", @"I don’t like Films", nil];
}
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    selectionArray = [AppDelegate sharedAppDelegate].currUser[@"FILMS"];
...
- (void)clickMusic
{
    [AppDelegate sharedAppDelegate].currUser[@"FILMS"] = selectionArray;
    [self performSegueWithIdentifier:@"ShowMusicView" sender:nil];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (realIndex < namesArray.count) {
        ...
        cell.firstIconView.image = [UIImage imageNamed:[NSString stringWithFormat:@"f%d.png", realIndex]];
    ...
    if (realIndex < namesArray.count) {
        ...
        cell.secondIconView.image = [UIImage imageNamed:[NSString stringWithFormat:@"f%d.png", realIndex]];
    ...
    if (realIndex < namesArray.count) {
        ...
        cell.thirdIconView.image = [UIImage imageNamed:[NSString stringWithFormat:@"f%d.png", realIndex]];
